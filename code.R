...
# ------------------------------------------------------------------------------
# REMOVE or COMMENT OUT THIS PART to RETAIN rows where score=3 (refusal).
# ------------------------------------------------------------------------------
# df <- df %>%
#   filter(
#     !apply(select(., all_of(score_columns)), 1, function(x) any(x == 3, na.rm = TRUE))
...
# ------------------------------------------------------------------------------
# COUNT refusals (3) as incorrect.
# ------------------------------------------------------------------------------
# binary_correct:   1 if score == 1; else 0 (includes 0, 0.5, or 3).
# binary_correct_partial: 1 if score == 1 or 0.5; else 0 (includes 0 or 3 as 0).
# ------------------------------------------------------------------------------
df_long <- df_long %>%
  mutate(
